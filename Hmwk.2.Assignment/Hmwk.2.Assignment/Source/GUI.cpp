...
void GUI::initGUI()
{
	...
	window = SDL_CreateWindow("Zelda 2 Clone",
...
void GUI::displayGameState(int linkPosX, int linkPosY, int linkSpriteID)
{
	//Clear screen
	SDL_SetRenderDrawColor(renderer, 100, 149, 237, 150);
	SDL_RenderClear(renderer);

	int adjustedX = linkPosX;
	if (linkSpriteID == 7)
	{
		adjustedX -= 6;
	}
	else if (linkSpriteID == 19)
	{
		adjustedX -= 16;
	}
	//Render link
	linkSheetTexture->render(renderer, adjustedX, linkPosY, &linkClips[linkSpriteID]);
