...
void GUI::initGUI() {
	...
	window = SDL_CreateWindow("MegaMan Clone",
...
void GUI::displayGameState(Link* link, Block** blocks, int numBlocks) {

	//Clear screen
	SDL_SetRenderDrawColor(renderer, 100, 149, 237, 150);
	...
	int adjustedX = link->getPosX();
	if (link->getSpriteID() == 7)
	{
		adjustedX -= 6;
	}
	else if (link->getSpriteID() == 19)
	{
		adjustedX -= 16;
	}

	//Render MegaMan
	linkSheetTexture->render(renderer, adjustedX, link->getPosY(), &linkClips[link->getSpriteID()]);
	...
	SDL_Rect renderQuad = { 450, 10, width, height };
