...
void GUI::loadMedia()
{
	...
	//Open Deeler Sprite Sheet
	portalSheetTexture = new Texture();
	portalSheetTexture->load(renderer,"./Assets/Images/deeler.png");
...
void GUI::displayGameState(Engine & engine)
{
	...
	for(int i=0;i<engine.getNumObjects();i++)
	{
		...
		{
			portalSheetTexture->render(renderer, object->plotX(), object->plotY(), NULL);
		}
	}
